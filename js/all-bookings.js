...
let customerPortalCustomerIds = new Set();
let currentTab = "all";
...
document.addEventListener("DOMContentLoaded", async () => {
    try {
        ...
        await Promise.all([loadDrivers(), loadBookings(), loadAccountCustomers(), loadCustomerPortalUsers(), loadAllBookingsSettings(), loadUnavailableDrivers()]);
        startLiveRefresh();
...
function bindBookingEvents() {
    ...
    document.getElementById("updateBookingDriver")?.addEventListener("click", updateBookingDriverDetails);
...
async function loadCustomerPortalUsers() {
    const { data, error } = await bookingsDb.from("customer_users").select("customer_id").eq("company_id", adminCompanyId);
    customerPortalCustomerIds = new Set(error ? [] : (data || []).map(row => String(row.customer_id)));
}

async function loadAllBookingsSettings() {
    const { data } = await bookingsDb.from("settings").select("drivercommission,currencysymbol").eq("company_id", adminCompanyId).maybeSingle();
    quoteSettings = data || {};
}
...
async function loadUnavailableDrivers() {
    const now = new Date().toISOString();
    const { data, error } = await bookingsDb.from("driver_unavailability").select("driver_id").eq("company_id", adminCompanyId).eq("active", true).lte("from_datetime", now).gte("to_datetime", now);
    unavailableDriverIds = new Set(error ? [] : (data || []).map(row => String(row.driver_id)));
}
...
async function loadBookings() {
    ...
    if (body) {
        ...
        body.innerHTML =
            '<tr><td colspan="15" class="empty-row">Loading bookings…</td></tr>';
    }
...
function renderBookings() {
    ...
    if (!rows.length) {
        ...
        body.innerHTML =
            '<tr><td colspan="15" class="empty-row">No bookings found for this view.</td></tr>';
...
function bookingRowHtml(booking) {
        ...
        ...allDrivers.map(driver => {
            ...
            const online =
                driver.online
                    ? " • ONLINE"
                    : "";
            const unavailable = unavailableDriverIds.has(String(driver.id));
            ...
            return `
                ...
                >
                    ${escapeHtml(
                        `${driver.driver_number || "-"} — ${driver.full_name || "Driver"}${online}${unavailable ? " • UNAVAILABLE" : ""}`
                    )}
                </option>
            `;
        })
    ...
    return `
        <tr class="${paymentClass}">
            ...
            <td data-label="Passengers">${escapeHtml(booking.passengers ?? "-")}</td>

            <td data-label="Vehicle">${escapeHtml(vehicleTierLabel(booking.vehicle_tier || booking.vehicle_type))}</td>
...
async function updateBookingDriverDetails() {
    const button = document.getElementById("updateBookingDriver");
    const booking = allBookings.find(row => String(row.id) === String(button?.dataset.bookingId));
    if (!booking) return;
    const driverId = document.getElementById("editBookingDriver")?.value || null;
    const amountValue = document.getElementById("editBookingDriverAmount")?.value ?? "";
    if (driverId && !allDrivers.some(driver => String(driver.id) === String(driverId))) return alert("Select a driver from this company.");
    if (driverId && unavailableDriverIds.has(String(driverId)) && String(driverId) !== String(booking.driver_id || "")) return alert("That driver is currently unavailable.");
    if (amountValue !== "" && (!Number.isFinite(Number(amountValue)) || Number(amountValue) < 0)) return alert("Enter a valid Driver Amount.");
    const previousDriverId = booking.driver_id || null;
    const driverChanged = driverId !== previousDriverId;
    const update = { driver_id: driverId, driver_amount: amountValue === "" ? null : Number(amountValue) };
    if (driverChanged) {
        update.dispatched_at = driverId ? new Date().toISOString() : null;
        if (!["accepted", "on_way", "passenger_onboard", "completed", "cancelled", "canceled", "no_show"].includes(bookingStatus(booking))) update.status = driverId ? "assigned" : "waiting";
    }
    const { data: updated, error } = await bookingsDb.rpc("admin_update_booking_driver", { target_company_id: adminCompanyId, target_booking_id: booking.id, target_driver_id: driverId, target_driver_amount: update.driver_amount });
    if (error || !updated) return alert(error?.message || "Booking was not found for this company.");
    if (driverChanged) {
        await requestGoogleCalendarSync(booking.id);
        await requestBookingEmailEvent(booking.id, "driver_assignment", { previous_driver_id: previousDriverId, event_id: `assignment:${Date.now()}` });
    }
    Object.assign(booking, update);
    setText("viewDriver", driverDisplayName(driverId));
    setText("viewDriverAmount", update.driver_amount == null ? "Not set — existing fare behavior" : money(update.driver_amount));
    renderBookings();
}
...
async function openBookingView(id) {
    ...
    setText("viewCustomerPortal", customerPortalCustomerIds.has(String(booking.customer_id || "")) ? "Online Account" : "Guest");
    const driverEditor = document.getElementById("editBookingDriver");
    if (driverEditor) {
        driverEditor.innerHTML = '<option value="">Unassigned</option>' + allDrivers.map(driver => `<option value="${escapeHtml(driver.id)}" ${unavailableDriverIds.has(String(driver.id)) && String(driver.id) !== String(booking.driver_id || "") ? "disabled" : ""}>${escapeHtml(`${driver.driver_number || "-"} — ${driver.full_name || "Driver"}${unavailableDriverIds.has(String(driver.id)) ? " • UNAVAILABLE" : ""}`)}</option>`).join("");
        driverEditor.value = booking.driver_id || "";
    }
    const driverAmountEditor = document.getElementById("editBookingDriverAmount");
    if (driverAmountEditor) driverAmountEditor.value = booking.driver_amount == null ? "" : booking.driver_amount;
    const updateDriverButton = document.getElementById("updateBookingDriver");
    if (updateDriverButton) updateDriverButton.dataset.bookingId = booking.id;
    ...
    setText("viewJourneyFare", money(booking.journey_fare ?? booking.price ?? booking.job_price));
    setText("viewCardFee", money(booking.card_booking_fee_amount || 0));
    const driverBase = Number(booking.driver_amount ?? booking.price ?? booking.job_price ?? 0);
    setText("viewDriverCommission", `${Number(quoteSettings.drivercommission || 0).toFixed(2)}% (${money(driverBase * Number(quoteSettings.drivercommission || 0) / 100)})`);
...
async function saveBookingDriverAmount() {
    ...
    const { data, error } = await bookingsDb.rpc("admin_update_booking_driver", {
        target_company_id: adminCompanyId,
        target_booking_id: bookingId,
        target_driver_id: booking?.driver_id || null,
        target_driver_amount: raw === "" ? null : Number(raw)
    });
    button.disabled = false;
    if (error || !data) return alert(error?.message || "No matching company booking was updated.");
    await loadBookings();
    openBookingView(bookingId);
}
